                    ...
                    <div className="lg:col-span-8 space-y-8">
                        ...
                        >
                            <div className="flex items-center justify-between mb-8">
                                <h3 className="text-xl font-black text-gray-900 dark:text-white tracking-tight">Profile Setup</h3>
                                <button 
                                    onClick={() => setIsEditModalOpen(true)}
                                    className="h-10 px-6 bg-blue-600 hover:bg-blue-700 text-white font-black text-[10px] uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-blue-500/20"
                                >
                                    Edit Profile
                                </button>
                            </div>
                            ...
                            <div className="flex items-center gap-6 mb-8">
                                <div className="w-24 h-24 bg-gradient-to-tr from-blue-500 to-indigo-600 rounded-[1.8rem] flex items-center justify-center text-4xl font-black text-white shadow-2xl">
                                    {user?.name?.[0] || "U"}
                                </div>
                                <div>
                                    <h4 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">{user?.name || "User"}</h4>
                                    <p className="text-gray-500 font-medium mt-1">{user?.email}</p>
                                    <div className="mt-3 px-3 py-1 bg-emerald-500/10 text-emerald-500 w-fit rounded text-[10px] font-black uppercase tracking-widest">
                                        {user?.plan || "PRO"} PLAN
                                    </div>
                                </div>
                            </div>
                        ...
                        >
                            ...
                            <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                                {loadingPages ? (
                                    ...
                                    <div className="p-6 text-center border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-3xl">
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">No pages connected</p>
                                    </div>
                                ) : (
                                    pages.map(page => (
                                        <div key={page.id} className="flex items-center gap-4 p-4 bg-black/5 dark:bg-black/20 rounded-2xl border border-white/5">
                                            <img src={page.picture} alt="" className="w-10 h-10 rounded-xl object-cover" />
                                            <div className="flex-1 min-w-0">
                                                <p className="font-black text-xs text-gray-900 dark:text-white truncate">{page.name}</p>
                        ...
                        >
                            ...
                            <div className="p-6 rounded-[2rem] bg-black/5 dark:bg-black/20 border border-white/5 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="font-black text-[10px] uppercase tracking-widest text-gray-900 dark:text-white">Dark Mode</p>
