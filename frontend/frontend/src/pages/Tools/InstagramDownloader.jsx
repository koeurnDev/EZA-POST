...
import { 
    Instagram, Download, Loader2, CheckCircle, Search, 
    Image as ImageIcon, Video, Layers, X, Sparkles, Zap
} from "lucide-react";
...
export default function InstagramDownloader() {
    ...
    const handleDownload = async () => {
        if (!url.includes("instagram.com")) {
            return toast.error("Invalid Instagram Link");
        }
        setLoading(true);
        setResult(null);
        try {
            ...
            if (res.data.success) {
                setResult(res.data);
                toast.success("Media found!", { icon: "✨" });
            }
        } catch (err) {
    ...
    return (
        <DashboardLayout>
            {/* 🌈 Modern Background Mesh (Instagram Gradient) - Optimized for Mobile */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 hidden md:block">
                <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-gradient-to-br from-purple-500/20 to-pink-500/20 rounded-full blur-[120px] animate-pulse" />
            ...
            >
                ...
                <div className="text-center space-y-3">
                    <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-500/10 border border-pink-500/20 text-pink-600 dark:text-pink-400 text-xs font-bold uppercase tracking-wider">
                        <Instagram size={14} /> Instagram Saver
                    </motion.div>
                    <motion.h1 variants={itemVariants} className="text-3xl md:text-5xl font-black tracking-tight text-gray-900 dark:text-white">
                        IG <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 via-pink-600 to-orange-500">Premium</span>
                    </motion.h1>
                    <motion.p variants={itemVariants} className="text-gray-500 dark:text-gray-400 max-w-md mx-auto text-sm md:text-base px-4">
                        Download Reels, Posts, and IGTV videos in original quality instantly.
                    </motion.p>
                </div>

                {/* 🔍 Search Input */}
                <div className="max-w-2xl mx-auto relative group px-2">
                    <div className="absolute -inset-1 bg-gradient-to-r from-purple-600 via-pink-600 to-orange-500 rounded-2xl blur opacity-25 group-focus-within:opacity-50 transition duration-500" />
                    <div className="relative flex items-center bg-white dark:bg-gray-900/80 backdrop-blur-2xl rounded-2xl border border-white/20 dark:border-white/10 shadow-2xl overflow-hidden">
                        ...
                        <div className="flex items-center gap-1 md:gap-2 pr-2 md:pr-4 shrink-0">
                            ...
                            <button 
                                ...
                                className="px-4 md:px-6 py-2 md:py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-90 text-white rounded-xl font-bold text-xs md:text-sm transition-all flex items-center gap-2 disabled:opacity-50 shadow-lg shadow-pink-500/20"
                            >
                                        ...
                                        <div className="aspect-[3/4] rounded-2xl overflow-hidden bg-black shadow-2xl relative border border-white/20">
                                            ...
                                            <div className="absolute top-4 left-4">
                                                <div className="px-3 py-1.5 bg-black/60 backdrop-blur-md rounded-lg border border-white/20 text-[10px] font-black text-white flex items-center gap-1.5 shadow-lg tracking-tighter uppercase">
                                                    {result.meta?.type === 'video' ? <><Video size={12} className="text-pink-400" /> REEL</> : <><ImageIcon size={12} className="text-pink-400" /> POST</>}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1 flex flex-col min-w-0 py-2">
                                        <div className="space-y-4 mb-6 md:mb-8">
                                            <div className="flex items-center justify-between">
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-pink-500/10 text-pink-600 dark:text-pink-400 text-[10px] font-bold uppercase tracking-widest border border-pink-500/20">
                                                    <CheckCircle size={12} /> Media Verified
                                                </span>
                                                <button onClick={clearResult} className="text-gray-400 hover:text-pink-500 transition-colors">
                                                    <X size={20} />
                                                </button>
                                            </div>
                                            <h2 className="text-lg md:text-2xl font-black text-gray-900 dark:text-white leading-tight line-clamp-2 md:line-clamp-3">
                                                {result.meta?.title || "Instagram Content"}
                                            </h2>
                                            <div className="flex flex-wrap gap-2">
                                                {['Original Res', 'Direct Link', 'Fast Server'].map(tag => (
                                        ...
                                        <div className="mt-auto grid grid-cols-1 gap-3">
                                            <a
                                                href={result.url}
                                                download
                                                className="h-12 md:h-14 bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-90 text-white rounded-2xl font-black text-sm md:text-base shadow-xl transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                                            >
                                                <Download size={18} /> Save Media
                                            </a>
                ...
                {!result && !loading && (
                    <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 max-w-4xl mx-auto pt-8">
                        {[
                            { title: "Any Content", desc: "Reels, Posts, IGTV, and Photos", icon: <Layers size={18} className="text-pink-500" /> },
                            { title: "Full Resolution", desc: "Highest quality available", icon: <Zap size={18} className="text-orange-500" /> },
                            { title: "One-Click Save", desc: "Direct downloads, no redirections", icon: <Sparkles size={18} className="text-purple-500" /> }
                        ].map((item, i) => (
