...
import { 
    Facebook, Download, Loader2, CheckCircle, Search, 
    Video, Image as ImageIcon, ExternalLink, X, Sparkles, Zap
} from "lucide-react";
...
export default function FacebookDownloader() {
    ...
    const getQualityLabel = (url) => {
        if (!url) return "Standard Quality";
    ...
    return (
        <DashboardLayout>
            {/* 🌈 Modern Background Mesh (Facebook Blue) - Optimized for Mobile */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 hidden md:block">
                <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-gradient-to-br from-blue-500/20 to-indigo-500/20 rounded-full blur-[120px] animate-pulse" />
                <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-gradient-to-tl from-sky-500/20 to-blue-500/20 rounded-full blur-[120px] animate-pulse delay-700" />
            </div>
            ...
            >
                ...
                <div className="text-center space-y-3">
                    <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-wider">
                        <Facebook size={14} /> Facebook Saver
                    </motion.div>
                    <motion.h1 variants={itemVariants} className="text-3xl md:text-5xl font-black tracking-tight text-gray-900 dark:text-white">
                        FB <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Premium</span>
                    </motion.h1>
                    <motion.p variants={itemVariants} className="text-gray-500 dark:text-gray-400 max-w-md mx-auto text-sm md:text-base px-4">
                        Download public Facebook videos in high-definition quality instantly.
                    </motion.p>
                </div>
                ...
                <div className="max-w-2xl mx-auto relative group px-2">
                    ...
                    <div className="relative flex items-center bg-white dark:bg-gray-900/80 backdrop-blur-2xl rounded-2xl border border-white/20 dark:border-white/10 shadow-2xl overflow-hidden">
                        <div className="pl-4 md:pl-6 text-gray-400 shrink-0">
                            <Search size={20} className="group-focus-within:text-blue-500 transition-colors" />
                        </div>
                        <input
                            type="text"
                            value={url}
                        ...
                        <div className="flex items-center gap-1 md:gap-2 pr-2 md:pr-4 shrink-0">
                            {url && (
                                <button onClick={() => setUrl("")} className="p-1 md:p-2 text-gray-400 hover:text-blue-500 transition-colors">
                                    <X size={18} />
                                </button>
                            )}
                            <button 
                                ...
                                className="px-4 md:px-6 py-2 md:py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs md:text-sm transition-all flex items-center gap-2 disabled:opacity-50 shadow-lg shadow-blue-500/20"
                            >
                    ...
                    {result && (
                        ...
                        >
                            <div className="bg-white/70 dark:bg-gray-900/50 backdrop-blur-3xl rounded-3xl p-4 md:p-8 border border-white/30 dark:border-white/10 shadow-2xl relative overflow-hidden group">
                                <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 rounded-full blur-[80px] -mr-32 -mt-32 transition-transform group-hover:scale-110 duration-1000" />
                                    ...
                                    <div className="w-full md:w-80 shrink-0">
                                        <div className="aspect-[3/4] rounded-2xl overflow-hidden bg-black shadow-2xl relative border border-white/20">
                                            {result.video?.type === 'image' ? (
                                                ...
                                                <video
                                                    src={result.video?.url || result.url}
                                            ...
                                            <div className="absolute top-4 left-4">
                                                <div className="px-3 py-1.5 bg-black/60 backdrop-blur-md rounded-lg border border-white/20 text-[10px] font-black text-white flex items-center gap-1.5 shadow-lg tracking-tighter uppercase">
                                                    {result.video?.type === 'image' ? <><ImageIcon size={12} className="text-blue-400" /> IMAGE</> : <><Video size={12} className="text-blue-400" /> {getQualityLabel(result.video?.url || result.url)}</>}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1 flex flex-col min-w-0 py-2">
                                        <div className="space-y-4 mb-6 md:mb-8">
                                            <div className="flex items-center justify-between">
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[10px] font-bold uppercase tracking-widest border border-blue-500/20">
                                                    <CheckCircle size={12} /> Media Ready
                                                </span>
                                                <button onClick={clearResult} className="text-gray-400 hover:text-blue-500 transition-colors">
                                                    <X size={20} />
                                                </button>
                                            </div>
                                            <h2 className="text-lg md:text-2xl font-black text-gray-900 dark:text-white leading-tight line-clamp-2 md:line-clamp-3">
                                                {result.meta?.title || "Facebook Content"}
                                            </h2>
                                            <div className="flex flex-wrap gap-2">
                                                {['Original Source', 'Direct Stream', 'Public Video'].map(tag => (
                                        ...
                                        <div className="mt-auto grid grid-cols-1 gap-3">
                                            <a
                                                href={result.url || result.video?.url}
                                                download
                                                className="h-12 md:h-14 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black text-sm md:text-base shadow-xl transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                                            >
                ...
                {!result && !loading && (
                    <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 max-w-4xl mx-auto pt-8">
                        {[
                            { title: "Pages & Groups", desc: "Download from any public source", icon: <Video size={18} className="text-blue-500" /> },
                            { title: "HD Playback", desc: "Supports up to 4K resolution", icon: <Zap size={18} className="text-sky-500" /> },
                            { title: "Secure Path", desc: "Direct links, no intermediate ads", icon: <Sparkles size={18} className="text-indigo-500" /> }
                        ].map((item, i) => (
